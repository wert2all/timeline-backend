...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := eq.withTags; query != nil {
		if err := eq.loadTags(ctx, query, nodes,
			func(n *Event) { n.Edges.Tags = []*Tag{} },
			func(n *Event, e *Tag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Event, init func(*Event), assign func(*Event, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(event.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(event.TagsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(event.TagsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(event.TagsPrimaryKey[1]))
