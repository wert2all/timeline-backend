...
func (tq *TagQuery) QueryEvent() *EventQuery {
...
func (tq *TagQuery) WithEvent(opts ...func(*EventQuery)) *TagQuery {
	query := (&EventClient{config: tq.config}).Query()
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withEvent; query != nil {
		if err := tq.loadEvent(ctx, query, nodes,
			func(n *Tag) { n.Edges.Event = []*Event{} },
			func(n *Tag, e *Event) { n.Edges.Event = append(n.Edges.Event, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadEvent(ctx context.Context, query *EventQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Event)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.EventTable)
		s.Join(joinT).On(s.C(event.FieldID), joinT.C(tag.EventPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.EventPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.EventPrimaryKey[0]))
